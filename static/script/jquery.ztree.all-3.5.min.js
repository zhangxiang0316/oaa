...
(function(k) {
	var E, F, G, H, I, J, r = {}, K = {}, s = {}, L = {
			...
			function(b) {
				...
				a.bind(c.NODECREATED, function(a, c, h) {
					j.apply(b.callback.onNodeCreated, [c, h, a])
				});
			...
			function(b) {
				...
				if (j.eqs(b.type, "mousedown")) l = "mousedown";
					...
					if (j.eqs(a.tagName, "span") && a.getAttribute("treeNode" +
						e.id.SWITCH) !== null) d = (k(a).parent("li").get(0) || k(a).parentsUntil("li").parent().get(0)).id, h = "switchNode";
					else {
						if (q = j.getMDom(c, a, [{
							tagName: "a",
				...
				if (j.eqs(b.type, "dblclick") && (l = "dblclick", q = j.getMDom(c, a, [{
					tagName: "a",
					attrName: "treeNode" + e.id.A
		...
		g = {
			...
			getNextNode: function(b, a) {
				if (!a) return null;
				for (var c = b.data.key.children, d = a.parentTId ? a.getParentNode() : g.getRoot(b), f = 0, h = d[c].length - 1; f <= h; f++)
					if (d[c][f] === a) return f == h ? null : d[c][f + 1];
				return null
			},
			getNodeByParam: function(b, a,c, d) {
				if (!a || !c) return null;
				for (var f = b.data.key.children, h = 0, l = a.length; h < l; h++) {
					if(!a[h]) continue;//add zhangxr 
			...
			getNodeName: function(b, a) {
				return "" + a[b.data.key.text]
			},
			getNodeTitle: function(b, a) {
				return "" + a[undefined==a[b.data.key.title]?b.data.key.text:b.data.key.title]
			},
			getNodes: function(b) {
			...
			getPreNode: function(b, a) {
				if (!a) return null;
				for (var c = b.data.key.children, d = a.parentTId ? a.getParentNode() : g.getRoot(b), f = 0, h = d[c].length; f < h; f++)
			...
			removeSelectedNode: function(b, a) {
				for (var c = g.getRoot(b), d = 0, f = c.curSelectedList.length; d < f; d++)
					if (a === c.curSelectedList[d] || !g.getNodeCache(b, c.curSelectedList[d].tId)) c.curSelectedList.splice(d, 1), d--, f--
			},
			...
			transformToArrayFormat: function(b,
				a) {
				if (!a) return [];
				var c = b.data.key.children,
					d = [];
				if (j.isArray(a))
					for (var f = 0, h = a.length; f < h; f++) d.push(a[f]), a[f][c] && (d = d.concat(g.transformToArrayFormat(b, a[f][c])));
				else d.push(a), a[c] && (d = d.concat(g.transformToArrayFormat(b, a[c])));
				return d
			},
			transformTozTreeFormat: function(b, a) {
				var c, d, f = b.data.simpleData.idKey,
					h = b.data.simpleData.pIdKey,
				...
				if (j.isArray(a)) {
					...
					for (c = 0, d = a.length; c < d; c++){
						if(!a[c]) continue;//跳过节点为null情况 zhangxr 13/08/21
						g[a[c][f]] = a[c];
					} 
					for (c = 0, d = a.length; c < d; c++){
						if(!a[c]) continue;//跳过节点为null情况 zhangxr 13/08/21
	...
	F = function(b, a) {
		var c = g.getSetting(b.data.treeId),
			d = c.view.autoCancelSelected && b.ctrlKey && g.isSelectedNode(c, a) ? 0 : c.view.autoCancelSelected && b.ctrlKey && c.view.selectedMulti ? 2 : 1;
		...
		uCanDo: function() {
				...
				if (!b.data.keep.leaf || !a || a.isParent)
					if (j.isArray(c) || (c =
						...
						var f = k("#" + a.tId + e.id.SWITCH),
							h = k("#" + a.tId + e.id.ICON),
							l = k("#" + a.tId + e.id.UL);
						if (!a.open) i.replaceSwitchClass(a, f, e.folder.CLOSE), i.replaceIcoClass(a, h, e.folder.CLOSE), a.open = !1, l.css({
							display: "none"
						});
			...
			appendNodes: function(b, a, c, d, f, h) {
				...
				for (var e = [], j = b.data.key.children,
					...
					var o = c[k];
					if(!o) continue; //若节点为空 则跳过 zhangxr 13/08/21
					if (f) {
			...
			appendParentULDom: function(b, a) {
				var c = [],
					d = k("#" + a.tId),
					f = k("#" + a.tId + e.id.UL),
					h = i.appendNodes(b, a.level + 1, a[b.data.key.children], a, !1, !0);
			...
			asyncNode: function(b, a, c, d) {
				...
				var l = b.async.contentType == "application/json",
					p = l ? "{" : "",
					n = "";
				for (f = 0, h = b.async.autoParam.length; a && f < h; f++) {
					var m = b.async.autoParam[f].split("="),
						o = m;
					m.length > 1 && (o = m[1], m = m[0]);
					l ? (n = typeof a[m] == "string" ? '"' : "", p += '"' + o + ('":' + n + a[m]).replace(/'/g, "\\'") + n + ",") : p += o + ("=" + a[m]).replace(/&/g, "%26") + "&"
				}
				if(b.async.autoParamToJsonData){
					var autoParamJson = "{";
					if(b.async.otherParam && b.async.otherParam.jsonData)
						autoParamJson = b.async.otherParam.jsonData + ",";
					//说明autoParam有值，那么拼凑到jsonData中去
					if(p){
						var pSplit = p.split('&');
						autoParamJson = autoParamJson.replace('}','');
						for(var tmp = 0; tmp < pSplit.length && pSplit[tmp]; tmp++){
							autoParamJson += "'" + pSplit[tmp].split('=')[0] + "':'" + pSplit[tmp].split('=')[1] + "'";
						}
						autoParamJson += "}";
					    b.async.otherParam.jsonData = autoParamJson;
					    p = "";
					}
				}
				if (j.isArray(b.async.otherParam))
					for (f = 0, h = b.async.otherParam.length; f <
						h; f += 2) l ? (n = typeof b.async.otherParam[f + 1] == "string" ? '"' : "", p += '"' + b.async.otherParam[f] + ('":' + n + b.async.otherParam[f + 1]).replace(/'/g, "\\'") + n + ",") : p += b.async.otherParam[f] + ("=" + b.async.otherParam[f + 1]).replace(/&/g, "%26") + "&";
				else
					for (var r in b.async.otherParam) l ? (n = typeof b.async.otherParam[r] == "string" ? '"' : "", p += '"' + r + ('":' + n + b.async.otherParam[r]).replace(/'/g, "\\'") + n + ",") : p += r + ("=" + b.async.otherParam[r]).replace(/&/g, "%26") + "&";
			...
			expandCollapseNode: function(b, a, c, d, f) {
				...
				if (a) {
					...
					if (a.open == c) j.apply(f, []);
						...
						var c = k("#" + a.tId + e.id.UL),
							h = k("#" + a.tId + e.id.SWITCH),
							n = k("#" + a.tId + e.id.ICON);
			...
			expandCollapseSonNode: function(b, a, c, d, f) {
				var h = g.getRoot(b),
					...
					h = a ? a[e] : h[e],
					e = a ? !1 : d,
					j = g.getRoot(b).expandTriggerFlag;
				g.getRoot(b).expandTriggerFlag = !1;
				if (h)
					for (var k = 0, m = h.length; k < m; k++) h[k] && i.expandCollapseSonNode(b, h[k], c, e);
				g.getRoot(b).expandTriggerFlag = j;
				i.expandCollapseNode(b, a, c, d, f)
			},
			makeDOMNodeIcon: function(b, a, c) {
				var d = g.getNodeName(a, c);
				/*edit by chenyue,2014-03-26,使树可以传入html代码。*/
/*					d = a.view.nameIsHTML ? d : d.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");*/
				if (a.view.swichTree) {
					...
					b.push("<span id='", c.tId, e.id.ICON, "' title='' treeNode", e.id.ICON, " class='", i.makeNodeIcoClass(a, c), "' style='", i.makeNodeIcoStyle(a, c), "'></span><span id='", c.tId, e.id.SPAN, "'>", d, "</span><span id='", c.tId, e.id.SWITCH, "' title='' class='", i.makeNodeLineClass(a, c), "' treeNode", e.id.SWITCH, "></span>")
				}
			},
			makeDOMNodeLine: function(b, a, c) {
				if (a.view.swichTree) {
					...
					b.push("<span class='", i.makeNodeBottomLineClass(a, c), "'><span class='lineLeft'></span><span class='lineRight'></span></span><span class='", i.nodeTopLine(a, c), "'></span>");
				}
			},
			...
			makeDOMNodeNameAfter: function(b, a, c) {
				if (a.view.swichTree) {
					b.push("</a>")
				} else {
					b.push("</a><span class='", i.nodebottomLine(a, c), "'></span>")
				}
			},
			makeDOMNodeNameBefore: function(b, a, c) {
				var d = g.getNodeTitle(a, c),
					f = i.makeNodeUrl(a, c),
					h = i.makeNodeFontCss(a, c),
					l = [],
					k;
				for (k in h) l.push(k, ":", h[k],
					";");
				b.push("<a id='", c.tId, e.id.A, "' class='level", c.level, "' treeNode", e.id.A, ' onclick="', c.click || "", '" ', f != null && f.length > 0 ? "href='" + f + "'" : "", " target='", i.makeNodeTarget(c), "' style='", l.join(""), "' part='", c.part, "'");
			...
			makeNodeBottomLineClass: function(b, a, c) {
				var lineClass = [];
				if (b.view.showLine) {
					if (a.isFirstNode && a.isLastNode) {
						lineClass.push("oneNodeLine");
					} else if (a.isFirstNode && !a.isLastNode) {
						lineClass.push("leftNodeLine");
					} else if (a.isLastNode && !a.isFirstNode) {
						lineClass.push("rightNodeLine");
					} else {
						lineClass.push("centerNodeLine");
					}
				}
				return lineClass.join('_');
			},
			nodebottomLine: function(b, a, c) {
				var lineClass = [];
				if (b.view.showLine) {
					//alert("dd");
					if (a.isParent) {
						if (a.open) {
							lineClass.push("nodebottomLine");
						} else {
							lineClass.push("");
						}
					} else {
			...
			removeNode: function(b, a) {
				...
				if (g.getNodeCache(b, a.tId)) {
					...
					g.removeSelectedNode(b, a);
					for (var h = 0, l = f[d].length; h < l; h++)
						if (f[d][h].tId == a.tId) {
							f[d].splice(h, 1);
							break
						}
					...
					if (!b.data.keep.parent && h == 0) f.isParent = !1, f.open = !1, h = k("#" + f.tId + e.id.UL), l = k("#" + f.tId + e.id.SWITCH), j = k("#" + f.tId + e.id.ICON), i.replaceSwitchClass(f, l, e.folder.DOCU), i.replaceIcoClass(f,
						j, e.folder.DOCU), h.css("display", "none");
					else if (b.view.showLine && h > 0) {
						var n = f[d][h - 1],
							h = k("#" + n.tId + e.id.UL),
							l = k("#" + n.tId + e.id.SWITCH);
						j = k("#" + n.tId + e.id.ICON);
			...
			switchNode: function(setting, node) {
				...
				// 修复在取消数据加载时，return false +变为-的bug ， famingyuan ,2013-03-23
				if (node.open || !j.canAsync(setting, node)) {
					i.expandCollapseNode(setting, node, !node.open);
				} else if (setting.async.enable) {
					if (!i.asyncNode(setting, node)) {
						if (setting.view.NoExpand) {
	...
	k.fn.zTree = {
		...
		destroy: function(b) {
			if (b && b.length > 0) i.destroy(g.getSetting(b));
			else
		...
		init: function(b, a, c) {
			...
			d.treeObj.empty();
			r[d.treeId] = d;
			if (k.browser.msie && parseInt(k.browser.version) < 7) d.view.expandSpeed = "";
			g.initRoot(d);
			b = g.getRoot(d);
			...
			c = {
				...
				getNodeIndex: function(a) {
					if (!a) return null;
					for (var b = d.data.key.children, c = a.parentTId ? a.getParentNode() : g.getRoot(this.setting), e = 0, i = c[b].length; e < i; e++)
						if (c[b][e] == a) return e;
					return -1
				},
				...
				reAsyncChildNodes: function(a, b, c) {
					if (this.setting.async.enable) {
						...
						if (b == "refresh") {
							for (var b = this.setting.data.key.children, m = 0, q = a[b] ? a[b].length : 0; m < q; m++) g.removeNodeCache(d, a[b][m]);
...
(function(m) {
	...
	m.extend(!0, m.fn.zTree._z, {
		...
		view: {
			checkNodeRelation: function(c, a) {
				var b, d, g, l = c.data.key.children,
					h = c.data.key.checked;
				b = j.radio;
			...
			makeChkClass: function(c, a) {
				var b = c.data.key.checked,
					...
					b = c.check.chkStyle + "_" + (a[b] ? d.TRUE : d.FALSE) + "_" +
						e,
			...
			repairAllChk: function(c, a) {
				if (c.check.enable && c.check.chkStyle === j.checkbox.STYLE)
					for (var b = c.data.key.checked, d = c.data.key.children, g = f.getRoot(c), l = 0, h = g[d].length; l < h; l++) {
						var i = g[d][l];
						i.nocheck !== !0 && (i[b] = a);
			...
			repairSonChkDisabled: function(c, a, b) {
				if (a) {
					...
					if (a[d])
						for (var g = 0, l = a[d].length; g < l; g++) e.repairSonChkDisabled(c, a[d][g], b)
				}
			},
			...
			setParentNodeCheckBox: function(c, a, b, d) {
				var g = c.data.key.children,
					...
					h = m("#" + a.tId + j.id.CHECK);
				d || (d = a);
				f.makeChkFlag(c, a);
				a.nocheck !== !0 && a.chkDisabled !== !0 && (a[l] = b, e.setChkClass(c, h, a), c.check.autoCheckTrigger && a != d && a.nocheck !== !0 && c.treeObj.trigger(j.event.CHECK, [null, c.treeId,
					a
				]));
				if (a.parentTId) {
					h = !0;
					if (!b)
						for (var g = a.getParentNode()[g], i = 0, o = g.length; i < o; i++)
			...
			setSonNodeCheckBox: function(c, a, b, d) {
				if (a) {
					...
					var i = !1;
					if (a[g])
						for (var o = 0, k = a[g].length; o < k && a.chkDisabled !== !0; o++) {
							var n = a[g][o];
					...
					if (a !=
						...
						e.setChkClass(c, h, a);
						c.check.autoCheckTrigger && a != d && a.nocheck !== !0 && c.treeObj.trigger(j.event.CHECK, [null, c.treeId, a])
					}
				}
			}
		},
		...
		data: {
			...
			getCheckStatus: function(c,
				a) {
				if (!c.check.enable || a.nocheck) return null;
				var b = c.data.key.checked;
				return {
					checked: a[b],
			...
			getTreeCheckedNodes: function(c, a, b, d) {
				...
				for (var g = c.data.key.children, e = c.data.key.checked, h = b && c.check.chkStyle == j.radio.STYLE && c.check.radioType == j.radio.TYPE_ALL, d = !d ? [] : d, i = 0, o = a.length; i < o; i++) {
					if(!a[i]) continue;//add zhangxr 13/08/21
					if (a[i].nocheck !== !0 && a[i][e] == b && (d.push(a[i]),
						h)) break;
					f.getTreeCheckedNodes(c, a[i][g], b, d);
			...
			getTreeChangeCheckedNodes: function(c, a, b) {
				if (!a) return [];
				for (var d = c.data.key.children, g = c.data.key.checked, b = !b ? [] : b, e = 0, h = a.length; e < h; e++){
					if(!a[e]) continue;// add zhangxr 13/08/21
			...
			makeChkFlag: function(c, a) {
				if (a) {
					var b = c.data.key.children,
						...
						g = -1;
					if (a[b])
						for (var e = !1, h = 0, i = a[b].length; h < i; h++) {
							var f = a[b][h],
								k = -1;
							if(!f) continue;//zhangxr 13/08/21
							if (c.check.chkStyle ==
								j.radio.STYLE)
							...
							if (c.check.chkStyle == j.checkbox.STYLE) {
	...
	f.addInitProxy(function(c) {
		...
		if (k.eqs(c.type, "mouseover")) {
			if (b.check.enable &&
				k.eqs(a.tagName, "span") && a.getAttribute("treeNode" + j.id.CHECK) !== null) d = a.parentNode.id, l = "mouseoverCheck"
	...
	f.addBeforeA(function(c, a, b) {
		...
		c.check.enable && (f.makeChkFlag(c, a), c.check.chkStyle == j.radio.STYLE && c.check.radioType == j.radio.TYPE_ALL && a[d] && f.getRoot(c).radioCheckedList.push(a), b.push("<span ID='", a.tId, j.id.CHECK, "' class='", e.makeChkClass(c, a), "' treeNode", j.id.CHECK, a.nocheck === !0 ? " style='display:none;'" : "", "></span>"))
	});
	f.addZTreeTools(function(c, a) {
		a.checkNode = function(a, b, f, h) {
			...
			if (a.chkDisabled !== !0 && (b !== !0 && b !== !1 && (b = !a[i]), h = !! h, (a[i] !== b || f) && !(h && k.apply(this.setting.callback.beforeCheck, [this.setting.treeId, a], !0) == !1) && k.uCanDo(this.setting) && this.setting.check.enable && a.nocheck !== !0)) a[i] = b, b = m("#" + a.tId + j.id.CHECK), (f || this.setting.check.chkStyle === j.radio.STYLE) && e.checkNodeRelation(this.setting, a), e.setChkClass(this.setting,
				b, a), e.repairParentChkClassWithSelf(this.setting, a), h && c.treeObj.trigger(j.event.CHECK, [null, c.treeId, a])
		};
		...
		a.setChkDisabled = function(a, b) {
			b = !! b;
			e.repairSonChkDisabled(this.setting,
				a, b);
			b || e.repairParentChkDisabled(this.setting, a, b)
		};
		var b = a.updateNode;
	...
	e.createNodes = function(c, a, b, d) {
		...
		b && e.repairParentChkClassWithSelf(c,
			d)
	};
	var t = e.removeNode;
...
(function(k) {
			...
			onMousedownNode: function(b, a) {
				function c(b) {
					...
					if (z.dragFlag == 1) {
						...
						var s = o < g.edit.drag.borderMax && o > g.edit.drag.borderMin,
						...
						if (r && r.attr("id") != i.treeId) {
							...
							if (!f && !a && !C) {
								...
								if (w && w.isParent && v == d.move.TYPE_INNER && (b = !0, window.zTreeMoveTimer && window.zTreeMoveTargetNodeTId !== w.tId ? (clearTimeout(window.zTreeMoveTimer), window.zTreeMoveTargetNodeTId = null) : window.zTreeMoveTimer && window.zTreeMoveTargetNodeTId === w.tId && (b = !1), b)) window.zTreeMoveTimer = setTimeout(function() {
									v == d.move.TYPE_INNER && w && w.isParent && !w.open && (new Date).getTime() - J > i.edit.drag.autoOpenTime && h.apply(i.callback.beforeDragOpen, [i.treeId, w], !0) && (e.switchNode(i,
				...
				function q(b) {
					...
					if (z.dragFlag != 0) {
						z.dragFlag = 0;
						var a, l, j;
						for (a = 0, l = m.length; a < l; a++) j = m[a], j.isParent && z.dragNodeShowBefore[j.tId] && !j.open && (e.expandCollapseNode(g, j, !j.open), delete z.dragNodeShowBefore[j.tId]);
						y && y.remove();
						t && t.remove();
						...
						if (r) {
							...
							if (h.apply(g.callback.beforeDrop, [i.treeId, m, n, v, o], !0) != !1) {
								...
								a = function() {
									if (G) {
										if (!o)
											for (var b = 0, a = m.length; b < a; b++) e.removeNode(g, m[b]);
										if (v == d.move.TYPE_INNER) e.addNodes(i,
											n, s);
										else if (e.addNodes(i, n.getParentNode(), s), v == d.move.TYPE_PREV)
											for (b = 0, a = s.length; b < a; b++) e.moveNode(i, n, s[b], v, !1);
										else
											for (b = -1, a = s.length - 1; b < a; a--) e.moveNode(i, n, s[a], v, !1)
									} else if (o && v == d.move.TYPE_INNER) e.addNodes(i, n, s);
									else if (o && e.addNodes(i, n.getParentNode(), s), v != d.move.TYPE_NEXT)
										for (b = 0, a = s.length; b < a; b++) e.moveNode(i, n, s[b], v, !1);
									else
										for (b = -1, a = s.length - 1; b < a; a--) e.moveNode(i, n, s[a], v, !1);
									for (b = 0, a = s.length; b < a; b++) e.selectNode(i, s[b], b > 0);
									k("#" + s[0].tId).focus().blur()
								};
								v == d.move.TYPE_INNER &&
									h.canAsync(i, n) ? e.asyncNode(i, n, !1, a) : a();
	...
	k.extend(!0, k.fn.zTree._z, {
		...
		view: {
			...
			addHoverDom: function(b, a) {
				if (p.getRoot(b).showHoverDom) a.isHover = !0, b.edit.enable && (e.addEditBtn(b,
					a), e.addRemoveBtn(b, a)), h.apply(b.view.addHoverDom, [b.treeId, a])
			},
			cancelCurEditNode: function(b, a) {
				...
				if (f) {
					...
					else f[q] = j ? j : l.val(), a || b.treeObj.trigger(d.event.RENAME, [b.treeId, f, isChange]);
			...
			editNode: function(b, a) {
				...
				if (p.isSelectedNode(b, a) && c.curEditNode == a && a.editNameFlag) setTimeout(function() {
					...
					f.attr("value", a[q]);
					b.edit.editNameSelectAll ? h.inputSelect(f) : h.inputFocus(f);
			...
			moveNode: function(b, a, c, q, f, l) {
				...
				if (a != c && (!b.data.keep.leaf || !a || a.isParent ||
					q != d.move.TYPE_INNER)) {
					...
					var n;
					o ? n = o = b.treeObj : (!l && q == d.move.TYPE_INNER ? e.expandCollapseNode(b, a, !0, !1) : l || e.expandCollapseNode(b, a.getParentNode(), !0, !1), o = k("#" + a.tId),
						n = k("#" + a.tId + d.id.UL), o.get(0) && !n.get(0) && (n = [], e.makeUlHtml(b, a, n, ""), o.append(n.join(""))), n = k("#" + a.tId + d.id.UL));
					var m = k("#" + c.tId);
					...
					} else if (c.isLastNode) x = h[g].length - 1, t = h[g][x - 1], t.isLastNode = !0;
					else
					...
					} else a.isFirstNode && q == d.move.TYPE_PREV ? (j[g].splice(y, 0, c), o = a, o.isFirstNode = !1, c.parentTId = a.parentTId, c.isFirstNode = !0, c.isLastNode = !1) : a.isLastNode && q == d.move.TYPE_NEXT ?
						(j[g].splice(y + 1, 0, c), o = a, o.isLastNode = !1, c.parentTId = a.parentTId, c.isFirstNode = !1, c.isLastNode = !0) : (q == d.move.TYPE_PREV ? j[g].splice(y, 0, c) : j[g].splice(y + 1, 0, c), c.parentTId = a.parentTId, c.isFirstNode = !1, c.isLastNode = !1);
					p.fixPIdKeyValue(b, c);
					p.setSonNodeLevel(b, c.getParentNode(), c);
					e.setNodeLineIcos(b, c);
					e.repairNodeLevelClass(b, c, r);
					!b.data.keep.parent && h[g].length < 1 ? (h.isParent = !1, h.open = !1, a = k("#" + h.tId + d.id.UL), q = k("#" + h.tId + d.id.SWITCH), g = k("#" + h.tId + d.id.ICON), e.replaceSwitchClass(h, q, d.folder.DOCU),
						e.replaceIcoClass(h, g, d.folder.DOCU), a.css("display", "none")) : t && e.setNodeLineIcos(b, t);
		...
		data: {
			setSonNodeLevel: function(b, a, c) {
				if (c) {
					...
					if (c[d])
						for (var a = 0, f = c[d].length; a < f; a++) c[d][a] && p.setSonNodeLevel(b,
							c, c[d][a])
				}
			}
		}
	...
	p.addInitBind(function(b) {
		var a = b.treeObj,
			c = d.event;
		a.bind(c.RENAME, function(a, c, d, isChange) {
			h.apply(b.callback.onRename, [c, d, a, isChange])
		});
		a.bind(c.REMOVE, function(a, c, d) {
			h.apply(b.callback.onRemove, [c, d, a])
		});
	...
	p.addInitProxy(function(b) {
		...
		else if (h.eqs(b.type, "mousedown") &&
			(k = h.getMDom(c, a, [{
				tagName: "a",
				attrName: "treeNode" + d.id.A
	...
	p.addZTreeTools(function(b, a) {
		...
		a.copyNode = function(a, k, f, l) {
			...
			f == d.move.TYPE_INNER ? (k = function() {
				e.addNodes(b, a, [j], l)
			}, h.canAsync(b, a) ? e.asyncNode(b, a, l, k) : k()) : (e.addNodes(b, a.parentNode, [j], l),
				e.moveNode(b, a, j, f, !1, l));
			return j
		};
		...
		a.moveNode = function(a, q, f, l) {
			...
			h.canAsync(b, a) ? e.asyncNode(b, a, l, j) : j();
			return q
		};
	...
	e.makeNodeUrl = function(b, a) {
		return b.edit.enable ? null : R.apply(e,
			arguments)
	};
	var J = e.removeNode;
	...
	h.uCanDo = function(b, a) {
		var c = p.getRoot(b);
		return a && (h.eqs(a.type, "mouseover") || h.eqs(a.type, "mouseout") || h.eqs(a.type, "mousedown") || h.eqs(a.type, "mouseup")) ? !0 : !c.curEditNode && (N ? N.apply(e, arguments) : !0)
	}
})(jQuery);
/*
...
(function(i) {
	i.extend(!0, i.fn.zTree._z, {
		view: {
			clearOldFirstNode: function(c, b) {
				for (var a = b.getNextNode(); a;) {
					if (a.isFirstNode) {
						a.isFirstNode = !1;
						f.setNodeLineIcos(c, a);
						break
					}
					if (a.isLastNode) break;
					a = a.getNextNode()
				}
			},
			clearOldLastNode: function(c, b) {
				for (var a = b.getPreNode(); a;) {
					if (a.isLastNode) {
						a.isLastNode = !1;
						f.setNodeLineIcos(c, a);
						break
			...
			makeDOMNodeMainBefore: function(c, b, a) {
				c.push("<li ", a.isHidden ? "style='display:none;' " : "", "id='", a.tId, "' class='level", a.level,
					"' tabindex='0' hidefocus='true' treenode>")
			},
			showNode: function(c, b) {
				b.isHidden = !1;
				e.initShowForExCheck(c, b);
				i("#" + b.tId).show()
			},
			showNodes: function(c, b, a) {
				if (b && b.length != 0) {
					var d = {}, h, j;
					...
					for (var k in d) b = d[k][c.data.key.children], f.setFirstNodeForShow(c, b), f.setLastNodeForShow(c, b)
				}
			},
			hideNode: function(c, b) {
				b.isHidden = !0;
				b.isFirstNode = !1;
				b.isLastNode = !1;
				e.initHideForExCheck(c, b);
				f.cancelPreSelectedNode(c, b);
				i("#" + b.tId).hide()
			},
			hideNodes: function(c, b, a) {
				if (b && b.length != 0) {
					var d = {}, h, j;
					for (h = 0, j = b.length; h < j; h++) {
						var g = b[h];
					...
					for (var k in d) b = d[k][c.data.key.children], f.setFirstNodeForHide(c, b), f.setLastNodeForHide(c, b)
				}
			},
			setFirstNode: function(c, b) {
				var a = c.data.key.children,
					d = b[a].length;
				d > 0 && !b[a][0].isHidden ?
					b[a][0].isFirstNode = !0 : d > 0 && f.setFirstNodeForHide(c, b[a])
			},
			setLastNode: function(c, b) {
				var a = c.data.key.children,
					d = b[a].length;
				d > 0 && !b[a][0].isHidden ? b[a][d - 1].isLastNode = !0 : d > 0 && f.setLastNodeForHide(c, b[a])
			},
			setFirstNodeForHide: function(c, b) {
				var a, d, h;
				for (d = 0, h = b.length; d < h; d++) {
					a = b[d];
					if(!a) continue;//add zhangxr 13/08/21
					if (a.isFirstNode) break;
					if (!a.isHidden && !a.isFirstNode) {
						a.isFirstNode = !0;
						f.setNodeLineIcos(c, a);
						break
					} else a = null
			...
			setFirstNodeForShow: function(c, b) {
				var a, d, h, e, g;
				for (d = 0, h = b.length; d < h; d++)
					if (a = b[d], !e && !a.isHidden &&
						a.isFirstNode) {
						e = a;
						break
					} else
				if (!e && !a.isHidden && !a.isFirstNode) a.isFirstNode = !0, e = a, f.setNodeLineIcos(c, a);
				else if (e && a.isFirstNode) {
					a.isFirstNode = !1;
					g = a;
					f.setNodeLineIcos(c, a);
					break
			...
			setLastNodeForHide: function(c, b) {
				var a, d;
				for (d = b.length - 1; d >= 0; d--) {
					a = b[d];
					if(!a) continue;//add zhangxr
					if (a.isLastNode) break;
					if (!a.isHidden && !a.isLastNode) {
						a.isLastNode = !0;
						f.setNodeLineIcos(c, a);
						break
					} else a = null
			...
			setLastNodeForShow: function(c, b) {
				var a, d, e, j;
				for (d = b.length - 1; d >= 0; d--)
					if (a = b[d], !e && !a.isHidden &&
						a.isLastNode) {
				...
				if (!e && !a.isHidden && !a.isLastNode) a.isLastNode = !0, e = a, f.setNodeLineIcos(c, a);
				else if (e && a.isLastNode) {
					a.isLastNode = !1;
					j = a;
					f.setNodeLineIcos(c, a);
					break
		...
		data: {
			initHideForExCheck: function(c, b) {
				if (b.isHidden && c.check && c.check.enable) b._nocheck = !! b.nocheck, b.nocheck = !0, b.check_Child_State = -1, f.repairParentChkClassWithSelf && f.repairParentChkClassWithSelf(c, b)
			},
			initShowForExCheck: function(c, b) {
				if (!b.isHidden && c.check && c.check.enable) {
					b.nocheck = b._nocheck;
					delete b._nocheck;
					if (f.setChkClass) {
						var a = i("#" + b.tId + s.id.CHECK);
						f.setChkClass(c, a, b)
					}
					f.repairParentChkClassWithSelf && f.repairParentChkClassWithSelf(c, b)
				}
			}
		}
	});
	var k = i.fn.zTree,
		...
		f = k._z.view,
		e = k._z.data;
	e.addInitNode(function(c, b, a) {
		if (typeof a.isHidden == "string") a.isHidden = t.eqs(a.isHidden, "true");
		a.isHidden = !! a.isHidden;
		//觉得这个处理逻辑无用，他将初始为hidden并且ztree的check为true时的节点隐藏其前面的选择图标，by shenye 2014.10.22
		//e.initHideForExCheck(c, a)
	});
	e.addBeforeA(function() {});
	e.addZTreeTools(function(c, b) {
		b.showNodes = function(a, b) {
			f.showNodes(c, a, b)
		};
		b.showNode = function(a, b) {
			a &&
				f.showNodes(c, [a], b)
		};
		b.hideNodes = function(a, b) {
			f.hideNodes(c, a, b)
		};
		b.hideNode = function(a, b) {
			a && f.hideNodes(c, [a], b)
		};
		var a = b.checkNode;
		if (a) b.checkNode = function(c, e, f, g) {
			(!c || !c.isHidden) && a.apply(b, arguments)
		}
	});
	var l = e.initNode;
	e.tmpHideParent = -1;
	e.initNode = function(c, b, a, d, h, j, g) {
		if (e.tmpHideParent !== d) {
			e.tmpHideParent = d;
			var i = (d ? d : e.getRoot(c))[c.data.key.children];
			e.tmpHideFirstNode = f.setFirstNodeForHide(c, i);
			e.tmpHideLastNode = f.setLastNodeForHide(c, i);
			f.setNodeLineIcos(c, e.tmpHideFirstNode);
			f.setNodeLineIcos(c, e.tmpHideLastNode)
		}
		h = e.tmpHideFirstNode === a;
		j = e.tmpHideLastNode === a;
		l && l.apply(e, arguments);
		j && f.clearOldLastNode(c, a)
	};
	var m = e.makeChkFlag;
	if (m) e.makeChkFlag = function(c, b) {
		(!b || !b.isHidden) && m.apply(e, arguments)
	};
	var n = e.getTreeCheckedNodes;
	if (n) e.getTreeCheckedNodes = function(c, b, a, d) {
		if (b && b.length > 0) {
			var f = b[0].getParentNode();
			if (f && f.isHidden) return []
		}
		return n.apply(e, arguments)
	};
	var o = e.getTreeChangeCheckedNodes;
	if (o) e.getTreeChangeCheckedNodes = function(c, b, a) {
		if (b && b.length >
			0) {
	...
	var p = f.expandCollapseSonNode;
	if (p) f.expandCollapseSonNode = function(c, b, a, d, e) {
		(!b || !b.isHidden) && p.apply(f, arguments)
	};
	var q = f.setSonNodeCheckBox;
	if (q) f.setSonNodeCheckBox = function(c, b, a, d) {
		(!b || !b.isHidden) && q.apply(f, arguments)
	};
	var r = f.repairParentChkClassWithSelf;
	if (r) f.repairParentChkClassWithSelf = function(c, b) {
		(!b || !b.isHidden) && r.apply(f, arguments)
	}
})(jQuery);